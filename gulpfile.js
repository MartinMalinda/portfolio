...
function cdimg(path, width){
  return 'https://res.cloudinary.com/serenity-themes/image/upload/q_auto,f_auto,w_'+width+ '/' + path;
}

gulp.task('markdown', function(){
  return gulp.src('src/templates/partials/**/*.md')
    .pipe(markdownit({plugins: [['markdown-it-task-lists', {label: true}]]}))
...
gulp.task('images',['svgmin'], function(){
  return gulp.src('src/images/**/*.{jpg,png}')
    .pipe(cache(imagemin({ optimizationLevel: 3, progressive: true, interlaced: true }))) //gulpif isProduction?
    .pipe(gulp.dest('dist/images/'));
});
...
gulp.task('critical', function () {
  ...
  .pipe(gulpif(isProduction, critical({base: 'dist/', inline: true, minify:true, extract: true, css: ['dist/styles/app.css']})))
  .pipe(gulpif(isProduction, htmlmin({collapseWhitespace: true})))
...
gulp.task('build', function(cb) {
  return runSequence('markdown', ['templates', 'styles', 'scripts', 'images'], 'critical', 'public-dist', cb);
});
